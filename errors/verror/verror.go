...
func NewCode(code vcode.Code, text ...string) error {
	errText := ""
	if len(text) > 0 {
		errText = text[0]
	}
...
func NewCodef(code vcode.Code, format string, args ...interface{}) error {
	return &Error{
		stack: callers(),
		text:  fmt.Sprintf(format, args...),
		code:  code,
...
// The parameter <skip> specifies the stack callers skipped amount.
func NewCodeSkip(code vcode.Code, skip int, text ...string) error {
	errText := ""
	if len(text) > 0 {
		errText = text[0]
	}
...
// The parameter <skip> specifies the stack callers skipped amount.
func NewCodeSkipf(code vcode.Code, skip int, format string, args ...interface{}) error {
	return &Error{
		stack: callers(skip),
		text:  fmt.Sprintf(format, args...),
		code:  code,
...
// It returns nil if given err is nil.
func WrapCode(code vcode.Code, err error, text ...string) error {
	if err == nil {
		return nil
	}
	errText := ""
...
// It returns nil if given <err> is nil.
func WrapCodef(code vcode.Code, err error, format string, args ...interface{}) error {
	if err == nil {
		return nil
	}
	return &Error{
...
// It returns nil if given err is nil.
// The parameter <skip> specifies the stack callers skipped amount.
func WrapCodeSkip(code vcode.Code, skip int, err error, text ...string) error {
	if err == nil {
		return nil
	}
	errText := ""
...
// It returns nil if given err is nil.
// The parameter <skip> specifies the stack callers skipped amount.
func WrapCodeSkipf(code vcode.Code, skip int, err error, format string, args ...interface{}) error {
	if err == nil {
		return nil
	}
	return &Error{
...
func Code(err error) vcode.Code {
	if err != nil {
		if e, ok := err.(apiCode); ok {
			return e.Code()
		}
	}
	return vcode.CodeNil
}
